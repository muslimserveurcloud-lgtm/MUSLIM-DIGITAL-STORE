...
export async function resolveDownload(token: string, requestingUserId: string) {
  ...
  const signedUrl = await getSignedDownloadUrl(grant.productFile.storageKey, grant.productFile.fileName);
  ...
  return { signedUrl, fileName: grant.productFile.fileName };
}
